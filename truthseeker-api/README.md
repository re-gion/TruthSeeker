...
- 当前运行时默认以 Kimi 2.5 作为四 Agent 的原生多模态推理基座，并在调用时禁用 thinking；可通过 `KIMI_PROVIDER=official|coding` 手动选择官方 API 或 Kimi coding plan。Reality Defender、VirusTotal 和 Exa 作为专业工具接入，所有工具失败都必须结构化降级。
