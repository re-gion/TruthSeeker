...
- Collaboration triggers when the same target Agent has 3 recent low-confidence rounds and adjacent changes `< 0.08`; each phase is capped at 1 collaboration per detection (`CONSULTATION_MAX_SESSIONS_PER_PHASE`) and each session asks at most 3 questions (`CONSULTATION_MAX_QUESTIONS`).
- Stage division of labor is a hard boundary: Forensics only examines the evidence itself (AIGC detectors, Reality Defender, ASR via Groq Whisper or Baidu short-speech extreme API / video observation, VirusTotal); WHOIS/IP/DNS/domain provenance/public-intel search belong to OSINT. Challenger must not challenge an Agent for work owned by another phase, and out-of-scope issues are deterministically filtered.
- Cross-validation means forensics-perspective (forged or not) × OSINT-perspective (fake/malicious or not) Agent-to-Agent corroboration. Multi-tool retesting within one phase is optional reinforcement, never a quality defect or challenge ground.
- Video decomposition results carry explicit detection-scope labels: RD audio-track results are audio-dimension only (`detection_scope=video_audio_track`, `analysis_scope=audio_track_only`), keyframe Sightengine results cover the visual dimension (`analysis_scope=video_visual_keyframes`). Tool summaries, the OSINT upstream-verified-conclusions citation block, and LLM prompts must preserve these scope labels; probabilities must never be applied across dimensions (e.g. audio-track probability as video-visual forgery probability), and a successful keyframe result means the visual dimension IS covered — never claim it is missing.
- Downstream Agents must cite upstream conclusions that passed Challenger review ("上游已核验结论", injected into OSINT's LLM context, covering both image and text AIGC findings) instead of rebuilding divergent low-confidence judgments. OSINT reports receive a deterministic "上游已核验结论引用" block prepended by code; when it is present and the narrative does not contradict upstream numbers, Challenger must not challenge narrative attribution or citation granularity.
- OSINT search: queries include domain-reputation queries plus LLM-extracted brand/entity queries (max 3 total). A completed Exa search with zero hits is a valid negative result and floors OSINT confidence at the release threshold (0.8); only unavailable/failed searches stay low-confidence. Empty Exa results are not reused across rounds; hits are. Brand/entity existence judgments must cite actual search coverage — "not found" without an entity-specific search is invalid.
- RAG checklists are references, not tool mandates: verification actions demanded by experience/case RAG but unsupported by the configured tools (e.g. social media, app stores, business-registry checks when only Exa search is configured) are not defects, challenge grounds, or collaboration issues; the Agent only records "当前工具不支持该验证动作".
