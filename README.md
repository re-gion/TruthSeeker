...
TruthSeeker 是一个面向 CISCN 2026 的跨模态恶意 AIGC 鉴伪、情报溯源与人机协同研判系统。它不是单点检测器，而是把多模态大模型、专业鉴伪工具、OSINT、证据图谱、逻辑质询和人工专家回注串成一条可审计的研判链路。

当前代码库采用 **Fed-MBPR-compatible** 运行时架构：底层检测器保持可替换，项目文档中的 Fed-MBPR 训练/推理底座属于研究方向；仓库内当前没有独立 Fed-MBPR 训练服务。
...
- **专业工具 all-settled 调用**：Sightengine、Reality Defender、VirusTotal、Exa、WhoisXML、内部文本 AIGC 检测器均以结构化成功/降级/失败结果进入证据板。
...
- Python 3.11+
- Supabase 项目
- 可选外部服务 key：Kimi/Moonshot 或 MiMo、Sightengine、Reality Defender、VirusTotal、Exa、WhoisXML、SiliconFlow Embedding
...
- 媒体/情报工具：`SIGHTENGINE_*`、`REALITY_DEFENDER_API_KEY`、`VIRUSTOTAL_API_KEY`、`EXA_API_KEY`、`WHOISXML_API_KEY`
...
- `docs/KNOWN_GAPS.md`：已确认但尚未修复的上线、安全、迁移与测试缺口
- `docs/CLEANUP_CANDIDATES.md`：已清理项与需要负责人确认的归档/删除候选
- `task.md`：里程碑状态
- `lessons.md`：开发错误记录本
- `AGENTS.md`：AI 代理工作规则
...
- 部署配置、竞赛答辩 PPT 和演示视频仍未在仓库内完成。
- Fed-MBPR 训练/推理底座不是当前运行时代码的一部分。
- 多数外部检测、情报和 embedding 能力依赖第三方 key、额度和网络；失败时系统应结构化降级。
- 真实端到端验证需要 Supabase、Storage、外部 API key 和前后端同时运行。
- 当前仍存在上线前必须修复的 RLS、canonical 协作认证、Storage 初始化和历史迁移风险，详见 `docs/KNOWN_GAPS.md`。
- 仓库当前没有 `LICENSE`、`CONTRIBUTING.md` 或 `CODE_OF_CONDUCT.md`；若准备公开发布，建议补齐。
