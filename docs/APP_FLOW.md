...
- `challenger` 在三个阶段分别审查取证报告、溯源图谱、最终报告。它会读全局证据板和原始样本上下文。
- `commander` 生成最终鉴伪与溯源报告。报告阶段如果被 Challenger 打回，只重写报告，不重新打开取证或搜索工具链。
- 每阶段最多 3 轮，质量变化阈值为 0.08。达到阈值则视为收敛；达到轮次上限仍未完全解决时继续推进，但写入残留风险。
