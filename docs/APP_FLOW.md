...
用户一次最多上传 5 个检材文件。当前前端统一预检单文件 250MB 上限；后端是最终入口，会按 MIME、真实魔数和文本安全性执行更严格限制：

- 视频：250MB。
- 音频：20MB。
- 图片：50MB。
- 文本：5MB，仅支持安全文本扩展名和可打印文本内容。
...
- `forensics` 不再是“只看视听”的专家，而是电子取证 Agent。它基于 Agent LLM 多模态上下文读取所有样本，图片默认调用 Sightengine `genai` 做 AIGC 图片检测，音频由 Reality Defender 做合成/篡改检测，视频按 RD 免费套餐能力边界分解为画面关键帧 Sightengine 检测（`video_keyframe_aigc`）与音轨 RD 检测，工具摘要显式标注检测范围（如“仅视频音轨”“视频画面检测（关键帧抽样）”），概率不得跨维度套用；音频/视频检材并行调用 ASR（`audio_transcription`，由 `AUDIO_ASR_PROVIDER` 在 Groq Whisper 与百度云短语音识别极速版之间切换）做语义转写（视频先探测音轨，无音轨记录正常结论并跳过上传），转写文本用于校验音频语义与文本主题一致性，文本检材调用内部 `ai_text_detector` 做多信号文本 AIGC 概率分析，文件哈希和 IOC 调用 VirusTotal。系统主字段统一使用 `aigc_probability`、`is_aigc` 和 `aigc_score`，旧 `deepfake_*` 只用于读取历史快照的兼容 fallback。
- `osint` 回归情报溯源，读取取证证据、全局输入和脱敏搜索线索，调用 Exa API、VirusTotal、WhoisXML，并复用内部文本 AIGC 检测结果；`text_claim_extract` 只负责社工风险 claim、诱导话术、URL 和异常线索抽取，不输出文本 AIGC 概率，避免和 `ai_text_detector` 混淆。`text_claim_extract` 是纯本地规则执行（不调用 Agent LLM）：LLM 不可用时不会挂起超时，关键声明（品牌主体、链接、时间压力、诱导特征）由本地规则提取。Exa 查询由域名信誉查询与 LLM 实体抽取生成的品牌/机构/产品实体查询组成（总数仍受 `MAX_EXA_QUERIES=3` 约束，实体抽取超时或失败时回退既有查询源）；实体存在性判断必须以实际执行的实体独立检索为准。搜索正常执行完成但零命中属于有效负结果，置信度保底到放行线 `0.8`（仅搜索不可用/失败保持低置信），避免零命中案件被硬门槛锁死在打回循环；上一轮零命中的空结果在重跑轮次不复用、重新搜索，已有命中的结果继续复用。OSINT 必须引用取证阶段经质询核验的“上游已核验结论”（图像/音视频与文本 AIGC 检测摘要、is_aigc、取证置信度、音频 ASR 转写摘要等，由代码注入 LLM 上下文；视频场景下音轨与画面关键帧两个维度的结论一并注入，且各自携带检测范围标注；ASR 转写属于证据内容而非鉴伪结论）讨论检材真伪与音频语义一致性，不得独立重建分歧的低置信判断，复用取证工具结果时必须标注上游来源；报告开头由代码确定性注入“上游已核验结论引用”小节，保证引用必然存在。其自身置信度仍只衡量情报溯源工作质量。
...
人机协同不是普通聊天，也不是事后批注。它是自动流程遇到低置信僵局、证据冲突或工具能力上限时的人工证据回注机制。参与者可以是用户本人，也可以是受邀外部专家。

协同状态流：

```mermaid
flowchart TD
  H["同一目标连续 3 轮低置信 + 变化停滞"] --> A{"本任务首次触发?"}
  A -->|是| W["自动暂停 waiting_collaboration"]
  A -->|否| P["提示用户审批是否再次协同"]
  P -->|批准| W
  P -->|跳过本次| R["记录残留高质询风险，回到 Challenger 继续"]
  W --> I["创建本次协同会话/专家邀请"]
  I --> E["用户/专家提交轻结构化意见"]
  E --> S["用户结束协同，Commander 汇总摘要"]
  S --> C{"用户编辑确认摘要"}
...
- 首轮全量调用：图片进 Sightengine `genai`，音视频进 Reality Defender，音频/视频检材并行进 ASR 语义转写（Groq Whisper 或百度云短语音识别极速版，`AUDIO_ASR_ENABLED`/`AUDIO_ASR_PROVIDER` 控制，未配置对应 Key 时结构化降级），文本检材进内部 `ai_text_detector`，社工风险 claim 与 URL/异常线索进 `text_claim_extract`；所有媒体哈希、文本 IOC 和 OSINT 新发现 IOC 进 VirusTotal，URL/域名线索进 WhoisXML 查询 WHOIS、DNS Lookup 当前解析和 IP Geolocation 归属。工具层可以保留 provider 原始标签（如 `AI_GENERATED`），但报告、SSE 和持久化裁决不得把图片 AIGC 概率混写成 Deepfake 概率。
...
- `citations`: 来源 URL、检索时间、摘要、文件哈希、API 结果摘要和 `source_kind`；`source_kind` 可为 `uploaded_file`、`tool_result`、`external_source`、`case_rag`、`experience_rag`
- `quality`: 完整性、引用覆盖率、模型推断比例、Challenger 审查结果

无引用但来自模型推理的关系可以进入图谱，但必须标记 `model_inferred=true`，不能作为外部事实展示。
当前图谱质量硬门槛为 `citation_coverage >= 0.60` 且 `model_inferred_ratio <= 0.35`，低于门槛会拉低 Challenger 对 OSINT 的置信度并触发打回。

## 7. SSE 事件

检测流使用 `POST /api/v1/detect/stream` 返回 SSE。Canonical 协同事件使用 `collaboration_*`，前端仍兼容旧 `consultation_*`：
...
入库时的 `media_category` 按检材主导伪造介质推导，优先级为视频 > 音频 > 图像：含视频归 `video_forgery`，含音频（如音频+文本）归 `audio_forgery`，仅图像+文本归 `image_text_mixed`，纯图像/纯文本分别归 `image_forgery`/`text_generation`；文件清单缺少 modality 时按 `input_type` 的同名模态兜底。公开标题与摘要由 LLM 基于 `analysis_states.result_snapshot` 中的取证/情报溯源案情分析（含文本内容、ASR 转写）生成，prompt 明确禁止引用检测诉求或编造输入中不存在的主体情节，输出再经脱敏并清除网址、裸域名与残留空引号；LLM 不可用或解析失败时退回只含裁决、类别和置信度的确定性文案。历史案例可用 `python scripts/backfill_public_case_metadata.py --task-id <id> --apply`（或 `--all`）重推类别、标题和摘要并同步刷新 RAG chunks。

公开接口为 `GET /api/v1/cases`、`GET /api/v1/cases/{id}`、`POST /api/v1/cases/{id}/preview-url`、`GET /api/v1/cases/{id}/files/{file_id}/text` 和 `DELETE /api/v1/cases/{id}`。列表和详情匿名可读，只返回 `status='published'` 的真实案例；`GET /api/v1/cases/{id}` 也支持 `builtin-*` 内置案例详情。预览接口按需从原任务私有文件记录读取 Storage path 并生成 10 分钟 signed URL，数据库不保存永久公开链接。前端 `/cases` 支持全部、文本生成、图像伪造、图文混合、音频伪造、视频伪造分类筛选和分页，`/cases/[id]` 渲染 Markdown 研判报告，内置展示案例也可点击查看补齐后的 Markdown 报告。公开案例 Markdown 会过滤“关键证据”章节，只保留裁决、摘要、处置建议等对公众有解释价值的内容；检测台正式报告不受影响。
...
- 入库：协同摘要确认后前端展示待入库草稿，用户可编辑和删除草稿；确认入库时后端再次规范化、去重、写入经验表并创建向量块。
...
- 不把 Fed-MBPR 训练/推理底座写成已运行实现；当前代码是 Fed-MBPR-compatible 运行时架构，底层检测器未来可替换为 Fed-MBPR 模型服务。
